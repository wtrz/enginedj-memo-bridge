# Engine DJ → DN-S3700 ID3 Sync

A Windows-oriented manual GUI sync tool that reads Engine DJ's SQLite database **read-only** and writes DN-S3700 / Denon DJ Music Manager style ID3 data into the original MP3 files before an Engine DJ USB export.
...
- Engine saved loops can experimentally map to a numbered DN Auto Loop slot.
...
The captured DN-S3700 file proves the Auto Loop slot type, start and BPM fields. The exact encoding of the selected Auto Loop beat length is not yet isolated. Therefore saved-loop-to-numbered-slot mapping is labelled **experimental**. Exact Engine loop start/end is supported through the separate DN A/B loop mapping.
